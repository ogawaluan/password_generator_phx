defmodule PassGenerator do
  @moduledoc """
  Generate a random password depending on parameters, Module main function is `generate(options)`.
  ...
  The options are only four, `length`, `uppercase`, `numbers`, `symbols`
  """
  ...
  defp included_options(options) do
    Enum.filter(options, fn {_keys, value} ->
